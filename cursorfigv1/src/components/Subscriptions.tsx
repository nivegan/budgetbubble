...
import { transactionAPI, subscriptionAPI } from '../utils/api';
...
export function Subscriptions({ accessToken, householdId, isPersonalView }: SubscriptionsProps) {
  ...
  const loadTransactions = async () => {
    setLoading(true);
    try {
      // Load persisted subscriptions first
      try {
        const subData = await subscriptionAPI.getAll(householdId, accessToken);
        if (subData.subscriptions && subData.subscriptions.length > 0) {
          setSubscriptions(subData.subscriptions);
        }
      } catch (e) {
        console.log('Could not load persisted subscriptions, will auto-detect');
      }
      ...
      // Auto-detect new subscriptions from transactions
      if (!isPersonalView) {
        try {
          const detectResult = await subscriptionAPI.detect(householdId, accessToken);
          if (detectResult.subscriptions && detectResult.subscriptions.length > 0) {
            // Merge with existing subscriptions
            setSubscriptions(prev => {
              const existing = new Set(prev.map(s => s.name?.toLowerCase()));
              const newOnes = detectResult.subscriptions.filter((s: any) => !existing.has(s.name?.toLowerCase()));
              return [...prev, ...newOnes];
            });
          }
        } catch (e) {
          console.log('Auto-detection failed, using local detection');
