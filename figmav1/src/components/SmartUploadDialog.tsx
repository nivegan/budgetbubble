...
import { CheckCircle2, XCircle, Upload, ArrowRight } from 'lucide-react';
...
export function SmartUploadDialog({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(selectedFile, {
      ...
      complete: (results) => {
        ...
        // Find header row (heuristic: first row with > 2 cells that aren't purely numeric)
        let foundHeaderIndex = rows.findIndex(row => 
          row.filter(cell => cell && isNaN(Number(cell))).length > 2
        );
        if (foundHeaderIndex === -1) foundHeaderIndex = 0;
        setHeaderRowIndex(foundHeaderIndex);
        
        const fileHeaders = rows[foundHeaderIndex].map((h, i) => (h || "").trim() || `Column ${i + 1}`);
        const filePreview = rows.slice(foundHeaderIndex + 1, foundHeaderIndex + 6);
        ...
        for (const field of fieldsToMap) {
          ...
          if (type === 'transactions') {
            ...
            if (field.key === 'withdrawal') found = fileHeaders[lowerCaseHeaders.findIndex(h => h.includes('withdrawal') || h.includes('debit') || h.includes('w/d'))];
            ...
            if (field.key === 'avgPrice') found = fileHeaders[lowerCaseHeaders.findIndex(h => (h.includes('avg') || h.includes('average')) && (h.includes('price') || h.includes('cost')))];
            if (field.key === 'ltp') found = fileHeaders[lowerCaseHeaders.findIndex(h => h.includes('ltp') || (h.includes('current') || h.includes('closing') || h.includes('present')) && (h.includes('price') || h.includes('value')))];
            if (field.key === 'type') found = fileHeaders[lowerCaseHeaders.findIndex(h => h.includes('type') || h.includes('instrument'))];
          }
          if (found && fileHeaders.includes(found)) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // formData.append('type', type); // Type is implied by the endpoint
      formData.append('mapping', JSON.stringify(mapping)); // Send the user's mapping
      formData.append('skipRows', String(headerRowIndex + 1)); // Tell backend how many rows to skip
      formData.append('autoDetectHeaders', 'false'); // We are providing the mapping
      formData.append('dedupe', 'true');

      // Use the correct endpoint from api.tsx
      const endpoint = type === 'transactions' 
        ? '/transactions/upload' 
        : '/holdings/upload'; // This endpoint must exist on your server
        ...
        {step === 1 && (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-[#c1d3e0]">Step 1 of 2: Select a CSV file</DialogDescription>
            </DialogHeader>
        ...
        {step === 2 && (
          <>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="overflow-x-auto max-h-60 border border-[#577189] rounded">
                <Table>
                  <TableHeader>
                    <TableRow className="border-[#577189]">
                      {headers.map((h, i) => (
                        <TableHead key={i} className="text-slate-300 whitespace-nowrap">{h}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {preview.map((row, i) => (
                      <TableRow key={i} className="border-[#577189]">
                        {row.map((cell, j) => (
